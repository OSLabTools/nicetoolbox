...
class SGFilter:
    ...
    Attributes:
        window_size (int): The length of the filter window (i.e., the number of 
            coefficients). `window_size` must be a positive odd integer. The size of 
            the window will affect the smoothness of the output signal, with larger 
            windows providing smoother results but less sensitivity to small variations 
            in the input data.
        polyorder (int): The order of the polynomial used to fit the samples. 
            `polyorder` must be less than `window_size`. A higher polynomial order can 
            fit the data more closely, but if too high, it may lead to overfitting, 
            causing artifacts in the filtered signal.
    ...
    def apply(self, data, is_3d=False):
        ...
        Args:
            data (np.array): The input data to be filtered. The shape of the array 
                should be [#Persons, #Cameras, #Frames, #Keypoints, XYZ].
        ...
        Returns:
            np.array: The filtered data. The shape of the array will be 
        ...
        for person_idx in range(data.shape[0]):
            for camera_idx in range(data.shape[1]):
                # apply the Savitzky-Golay filter to each dimension (X, Y, (Z)) of 
                # each keypoint
