...
def compare_npz_files(prediction_file, expectation_file, description="", rtol=1e-3, atol=1e-1):
    ...
    return compare_dicts_of_general_nparrays(prediction, expectation, description, rtol, atol)


def compare_dicts_of_general_nparrays(prediction, expectation, description="", rtol=1e-3, atol=1e-1):
    is_equal, common_keys = compare_dict_keys(prediction, expectation, description)

    for key in common_keys:
        is_equal &= compare_numerical_nparrays(prediction[key], expectation[key], key, rtol=rtol, atol=atol)
...
def compare_dict_keys(dict1, dict2, description=""):
    ...
    if dict1_keys - dict2_keys != set():
        is_equal = False
        logger.debug(
            f"{description}: Keys {dict1_keys - dict2_keys} found " "in the first but not the second dictionary."
        )
    if dict2_keys - dict1_keys != set():
        is_equal = False
        logger.debug(
            f"{description}: Keys {dict2_keys - dict1_keys} found " "in the second but not the first dictionary."
        )
...
def compare_numerical_nparrays(pred, expect, description="", rtol=1e-3, atol=1e-1):
    if pred.shape != expect.shape:
        logger.debug(f"{description}: Shapes of arrays differ: {pred.shape} vs " f"{expect.shape}.")
        return False
    ...
    if expect.dtype.kind in {"i", "u", "f", "c"}:
        if not np.allclose(pred, expect, equal_nan=True, rtol=rtol, atol=atol):
            ...
            logger.debug(f"{description}: Arrays " f"differ by a mean absolute difference of {diff}.")
            return False
        ...
        logger.debug(f"{description}: Arrays of type {expect.dtype.kind} are not supported.")
