...
import utils.in_out as ut_in_out
...
class Data:
    ...
    def __init__(self, config, io, data_formats, all_camera_names, all_dataset_names) -> None:
        """
    ...
    def get_inference_path(self, detector_name):
        ...
        """
        filepath = os.path.join(self.code_folder, 'detectors', 'third_party', detector_name, 'inference.py')
        try:
    ...
    def get_venv_path(self, detector_name, env_name):
        ...
        if os_type == "linux":
            filepath = os.path.join(
                self.code_folder, 'detectors', 'third_party', 
                detector_name, f'{env_name}/bin/activate')
        elif os_type == "windows":
            filepath = os.path.join(
                self.code_folder, 'detectors', 'third_party', 
                detector_name, env_name, 'Scripts', 'activate')
        try:
            exc.file_exists(filepath)
        except FileNotFoundError:
            logging.exception(
    ...
    def create_inputs_from_video(self):
        ...
        for video_file in video_files:
            ...
            if 'segments' in self.data_formats:
                ...
                frames_per_segment = int(self.annotation_interval * vid.get_fps(
                        video_files[0]))
