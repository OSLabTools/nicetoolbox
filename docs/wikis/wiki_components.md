...
For visualization or further analysis, the algorithm also projects the fused 3D gaze direction back into each camera view, resulting in a 2D gaze direction per camera (see `…_gaze_2d.csv` and `gaze_2d.npy` file). This projection is a **unit-length** image-plane direction `(u, v)` — a direction only, not a pixel displacement and not a point. It is resolution-independent, so consumers that draw it as an arrow scale it by their own pixel length (the built-in visualizations use `image_width / 5`). These 2D projections are also smoothed during post-processing (see `…_gaze_2d_filtered.csv` or `gaze_2d_filtered.npy` file).
