    ...
    for session_folder in walk_directory(directory):
        ...
        for sequence_folder in sequence_folders:
            ...
            if sequence_name == 'None' or walk_directory(sequence_folder) == []:
                camera_names = find_video_files(sequence_folder) if sequence_folder != 'None' else find_video_files(session_folder)
            else:
...
def make_input_form(frame, main, entries):    
    ...
    button_dataset = tk.Button(io_dataset, text='Select', command=(lambda e=entries: select_dataset_directory(e)))
    button_dataset.pack(side=tk.LEFT, padx=px, pady=py)
    button_load_dataset = tk.Button(io_dataset, text='Load', command=(lambda e=(main, entries): load_dataset_directory(*e)))
    ...
    # create select and load buttons
    button_calibration = tk.Button(io_calibration, text='Select', command=(lambda e=entries: select_calibration_file(e)))
    button_calibration.pack(side=tk.LEFT, padx=px, pady=py)
    button_load_calibration = tk.Button(io_calibration, text='Load', command=(lambda e=(main, entries): load_calibration_file(*e)))
