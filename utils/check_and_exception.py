...
def check_options(object, object_type, options) -> None:
    ...
    if not isinstance(object, object_type):
        raise TypeError(f"Expected object of type {object_type.__name__}, got {type(object).__name__}")
...
def check_value_bounds(object, object_type=None, object_min=None, object_max=None) -> None:
    ...
    if object_max is not None and object > object_max:
        raise ValueError(f"Object value {object} is greater than the maximum allowed value {object_max}")
            ...
            if isinstance(cval, str):
                ...
                if cval.startswith('type'):
                    ...
                    if not isinstance(tval, __builtins__[check_type]):
                        error_log_and_raise(TypeError, config_name, f"Key '{tkey}' requires value of type {check_type}. The given value is '{tval}'.")
                    ...
                    if folder_details == 'base':
                        ...
                        error_log_and_raise(ValueError, config_name, f"Unknown check value {folder_details}. Options are 'base' or 'full'.")
                        
                    if not os.path.isdir(folder):
                        error_log_and_raise(NotADirectoryError, config_name, f"For key '{tkey}', the folder '{folder}' is not a directory.")

                elif cval.startswith('file'):
                    if not os.path.isfile(tval):
                        error_log_and_raise(FileNotFoundError, config_name, f"For key {tkey}, the file '{tval}' is not found.")
                    ...
                    error_log_and_raise(NotImplementedError, config_name, f"Check option '{cval.split(':')[0]}' is unknown. Currently supported options in strings are 'type', 'folder', 'file', 'keys', 'tbd'.")
                    
            # check given options
            elif isinstance(cval, list):
                if tval not in cval:
                    error_log_and_raise(ValueError, config_name, f"Key '{tkey}' can take values {cval}. The given value is '{tval}'.")
